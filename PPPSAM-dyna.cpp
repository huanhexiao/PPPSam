...
double gInitClkNoise, gInitAmNoise, gBetwPosNoise, gBetwTrpNoise, gPhNoiseFactor;
...
void UdStates(gtime_t CurTime, int CurCount, rtk_t *rtk,
  ...
  if (gbFirstEpoch) {
    ...
    /* temporal update of pos */
    // Values
    NomXYZ = gResultValues.at<Vector3>(X(CurCount - 1));
    gNewInitValues.insert(X(CurCount), NomXYZ);
    // Between facror
    DiagNoise::shared_ptr BetwPosNoise = DiagNoise::Variances(
        Vector3(gBetwPosNoise, gBetwPosNoise, gBetwPosNoise));
    gNewGraph->add(BetweenFactor<Vector3>(X(CurCount), X(CurCount - 1),
                                          Vector3::Zero(), BetwPosNoise));
...
void Procpos(const prcopt_t *sProcOpt, const solopt_t *sSolutionOption,
  ...
  while ((nobs = Inputobs(obs, rtk.sol.stat, sProcOpt)) >= 0) {
    ...
    for (auto &mpit : mCurPntData) {
      ...
      PhaseFactor PhFactor(X(CurCount), C(CurCount), T(CurCount), N(gAllGNKey_[PRN]),
                           PhObs, SatData, dr, AntData,
                           DiagNoise::Variances(Vector1(VarPh)));

      // Generate pseudorange factor
      PseudorangeFactor PrFactor(X(CurCount), C(CurCount), T(CurCount), PrObs, SatData,
                                 dr, AntData,
    ...
    // 位置估值及显示
    PropXYZ = gResultValues.at<Vector3>(X(CurCount));
...
int main(int argc, char *argv[]) {
  ...
  gBetwPosNoise = config["BetwPosNoise"].as<double>();
