...
static int decode_ssr7_head(rtcm_t *rtcm, int sys, int *sync, int *iod,
  ...
  *udint = ssrudint[udi];
  ...
  trace(4,
        "decode_ssr7_head: time=%s sys=%d nsat=%d sync=%d iod=%d provid=%d "
        "solid=%d\n",
        time_str(rtcm->time, 2), sys, nsat, *sync, *iod, provid, solid);
...
static const int codes_gps[] = {
    CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L2C,
...
static const int codes_bds[] = {CODE_L1I, CODE_L1Q, CODE_L1X,
...
static int decode_ssr7(rtcm_t *rtcm, int sys) {
  ...
  int i, j, k, type, mode, sync, iod, nsat, prn, sat, nbias, ncode, np, mw,
      offp, sii, swl;
  int dispe, sdc, yaw_ang, yaw_rate;

  type = getbitu(rtcm->buff, 24, 12);

  if ((nsat = decode_ssr7_head(rtcm, sys, &sync, &iod, &udint, &dispe, &mw,
                               &i)) < 0) {
    trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
    return -1;
  }
