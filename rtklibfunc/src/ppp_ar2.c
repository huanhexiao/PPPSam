...
#define NR(opt)                            \
  (IT(opt) + ((opt)->tropopt < TROPOPT_EST \
                  ? 0                      \
...
static double var_LC(int i, int j, int k, double sig) {
  ...
  return (SQR(i * f1) + SQR(j * f2) + SQR(k * f5)) /
         SQR(i * f1 + j * f2 + k * f5) * SQR(sig);
}
...
static double f_erfc(double x) {
  return x >= 0.0 ? q_gamma(0.5, x * x, LOG_PI / 2.0)
                  : 1.0 + p_gamma(0.5, x * x, LOG_PI / 2.0);
}
...
static void average_LC(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav,
  ...
  for (i = 0; i < n; i++) {
    ...
    sig = sqrt(SQR(rtk->opt.err[1]) +
               SQR(rtk->opt.err[2] / sin(azel[1 + 2 * i])));
    ...
    if (LC1) {
      ...
      amb->LCv[0] += (SQR(LC1 - amb->LC[0]) - amb->LCv[0]) / amb->n[0];
    }
    if (LC2) {
      amb->n[1] += 1.0;
      amb->LC[1] += (LC2 - amb->LC[1]) / amb->n[1];
      amb->LCv[1] += (SQR(LC2 - amb->LC[1]) - amb->LCv[1]) / amb->n[1];
    }
    if (LC3) {
      amb->n[2] += 1.0;
      amb->LC[2] += (LC3 - amb->LC[2]) / amb->n[2];
      amb->LCv[2] += (SQR(LC3 - amb->LC[2]) - amb->LCv[2]) / amb->n[2];
    }
    amb->epoch[0] = obs[0].time;
  }
}
/* fix wide-lane ambiguity ---------------------------------------------------*/
static int fix_amb_WL(rtk_t *rtk, const nav_t *nav, int sat1, int sat2,
                      int *NW) {
  ambc_t *amb1, *amb2;
  ...
  if (!amb1->n[0] || !amb2->n[0]) return 0;
    ...
    /* wide-lane ambiguity */
#ifndef REV_WL_FCB
  BW = (amb1->LC[0] - amb2->LC[0]) / lam_WL + nav->wlbias[sat1 - 1] -
       nav->wlbias[sat2 - 1];
#else
  BW = (amb1->LC[0] - amb2->LC[0]) / lam_WL - nav->wlbias[sat1 - 1] +
       nav->wlbias[sat2 - 1];
...
static int fix_sol(rtk_t *rtk, const int *sat1, const int *sat2,
  ...
  trace(5, "AR filtering v=\n");
  tracemat(5, v, 1, n, 8, 3);
  ...
  for (i = 0; i < rtk->na; i++) {
    rtk->xa[i] = rtk->x[i];
    for (j = 0; j < rtk->na; j++) {
      rtk->Pa[i + j * rtk->na] = rtk->Pa[j + i * rtk->na] =
          rtk->P[i + j * rtk->nx];
    }
  }
  /* set flags */
...
static int fix_amb_ROUND(rtk_t *rtk, int *sat1, int *sat2, const int *NW,
                         int n) {
  double C1, C2, B1, v1, BC, v, vc, *NC, *var, lam_NL = lam_LC(1, 1, 0), lam1,
                                               lam2;
  int i, j, k, m = 0, N1, stat = 0;
  lam1 = lam_carr[0];
  lam2 = lam_carr[1];
  ...
  for (i = 0; i < n; i++) {
    ...
    var[m] = rtk->P[j + j * rtk->nx] + rtk->P[k + k * rtk->nx] -
             2.0 * rtk->P[j + k * rtk->nx];
    v1 = var[m] / SQR(lam_NL);

    /* validation of narrow-lane ambiguity */
    if (fabs(N1 - B1) > rtk->opt.thresar[3] ||
    ...
    if (m == 1) trace(6, "\n                 WL    NL(N1)    NC\n");
    trace(6, " G%2d-G%2d :%7d%7d%11.5f\n", sat1[m - 1], sat2[m - 1], NW[i], N1,
          NC[m - 1]);
  }
  ...
  /* fixed solution */
  if (m > 3) stat = fix_sol(rtk, sat1, sat2, NC, m);

  free(NC);
  free(var);

  trace(6, "fix number of the current epoch:%2d\n", m);
  return stat && m >= 3;
}
...
static int fix_amb_ILS(rtk_t *rtk, int *sat1, int *sat2, int *NW, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    /* validation of narrow-lane ambiguity */
    if (fabs(N1[m] - B1[m]) > rtk->opt.thresar[3]) continue;
    ...
    if (m == 1) trace(6, "                   B1        N1    NW\n");
    trace(6, " G%2d-G%2d :%11.5f%9.2f%5d\n", sat1[m - 1], sat2[m - 1],
          B1[m - 1], N1[m - 1], NW[m - 1]);
  }
  if (m < 4) return 0;
  ...
  // trace(6, "float covariance matrix P:\n"); tracemat(6, rtk->P, rtk->nx,
  // rtk->nx, 11, 8); trace(6, "NL AMB covariance matrix Q:\n"); tracemat(6, Q,
  // m, m, 11, 8);

  /* integer least square */
  if ((info = lambda(m, 2, B1, Q, N1, s))) {
    trace(6, "lambda error: info=%d\n", info);
    return 0;
  }
  if (s[0] <= 0.0) return 0;
  ...
  if (rtk->opt.thresar[0] > 0.0 && rtk->sol.ratio < rtk->opt.thresar[0]) {
    trace(6, "varidation error: n=%2d ratio=%8.3f\n", m, rtk->sol.ratio);
    return 0;
  }
  ...
  if (rtk->nfix >= rtk->opt.minfix) {
    ...
    trace(6, "continous fix numbers: nfix=%4d\n", rtk->nfix);
  }
...
extern int ppp_ar2(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav,
                   const double *azel) {
  double elmask;
  ...
  for (i = 0; i < n - 1; i++)
    for (j = i + 1; j < n; j++) {
      if (!rtk->ssat[obs[i].sat - 1].vsat[0] ||
          !rtk->ssat[obs[j].sat - 1].vsat[0] || azel[1 + i * 2] < elmask ||
          azel[1 + j * 2] < elmask)
        continue;
#if 0
		/* test already fixed */
      ...
      if (fix_amb_WL(rtk, nav, sat1[m], sat2[m], NW + m)) {
        m++;
        /*if (m == 1)trace(6, "sat1-sat2(of WL):\n");
        trace(6, "  G%2d - G%2d \n", sat1[m - 1], sat2[m - 1]);*/
      }
    }
