...
/* sbas igp definition -------------------------------------------------------*/
static const short
    x1[] = {-75, -65, -55, -50, -45, -40, -35, -30, -25, -20, -15, -10, -5, 0,
    ...
    x4[] = {-85, -75, -65, -55, -50, -45, -40, -35, -30, -25,
            -20, -15, -10, -5,  0,   5,   10,  15,  20,  25,
            30,  35,  40,  45,  50,  55,  65,  75},
    x5[] = {-180, -175, -170, -165, -160, -155, -150, -145, -140, -135, -130,
    ...
    x6[] = {-180, -170, -160, -150, -140, -130, -120, -110, -100,
...
static double varfcorr(int udre) {
  const double var[14] = {0.052,  0.0924,  0.1444,   0.283,   0.4678,
                          0.8315, 1.2992,  1.8709,   2.5465,  3.326,
...
static double varicorr(int give) {
  const double var[15] = {0.0084, 0.0333, 0.0749, 0.1331, 0.2079,
                          0.2994, 0.4075, 0.5322, 0.6735, 0.8315,
...
static double degfcorr(int ai) {
  const double degf[16] = {0.00000, 0.00005, 0.00009, 0.00012, 0.00015, 0.00020,
...
static int decode_sbstype2(const sbsmsg_t *msg, sbssat_t *sbssat) {
  ...
  for (i = 0; i < 13; i++) {
    ...
    if (t0.time == 0 || dt <= 0.0 || 18.0 < dt ||
        sbssat->sat[j].fcorr.ai == 0) {
...
static int decode_sbstype9(const sbsmsg_t *msg, nav_t *nav) {
  ...
  i = msg->prn - MINPRNSBS;
  if (!nav->seph ||
      fabs(timediff(nav->seph[i].t0, seph.t0)) < 1E-3) { /* not change */
    return 0;
  }
...
static int decode_longcorr0(const sbsmsg_t *msg, int p, sbssat_t *sbssat) {
  ...
  for (i = 0; i < 3; i++) {
    sbssat->sat[n - 1].lcorr.dpos[i] =
        getbits(msg->msg, p + 14 + 9 * i, 9) * 0.125;
...
static int decode_longcorr1(const sbsmsg_t *msg, int p, sbssat_t *sbssat) {
  ...
  for (i = 0; i < 3; i++) {
    sbssat->sat[n - 1].lcorr.dpos[i] =
        getbits(msg->msg, p + 14 + i * 11, 11) * 0.125;
    sbssat->sat[n - 1].lcorr.dvel[i] =
        getbits(msg->msg, p + 58 + i * 8, 8) * P2_11;
  }
...
static int decode_sbstype25(const sbsmsg_t *msg, sbssat_t *sbssat) {
  ...
  return decode_longcorrh(msg, 14, sbssat) &&
         decode_longcorrh(msg, 120, sbssat);
}
...
static void readmsgs(const char *file, int sel, gtime_t ts, gtime_t te,
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    if (sscanf(buff, "%d %lf %d", &week, &tow, &prn) == 3 &&
        (p = strstr(buff, ": "))) {
      p += 2; /* rtklib form */
    } else if (sscanf(buff, "%d %lf %lf %lf %lf %lf %lf %d", &prn, ep, ep + 1,
                      ep + 2, ep + 3, ep + 4, ep + 5, &msg) == 8) {
    ...
    if (sbs->n >= sbs->nmax) {
      ...
      if (!(sbs_msgs =
                (sbsmsg_t *)realloc(sbs->msgs, sbs->nmax * sizeof(sbsmsg_t)))) {
 ...
 *          only the following file extentions after wild card expanded are
 *valid to read. others are skipped .sbs, .SBS, .ems, .EMS
 *-----------------------------------------------------------------------------*/
extern int sbsreadmsgt(const char *file, int sel, gtime_t ts, gtime_t te,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (strcmp(ext, ".sbs") && strcmp(ext, ".SBS") && strcmp(ext, ".ems") &&
        strcmp(ext, ".EMS"))
      continue;
...
extern void sbsoutmsg(FILE *fp, sbsmsg_t *sbsmsg) {
  ...
  fprintf(fp, "%4d %6d %3d %2d : ", sbsmsg->week, sbsmsg->tow, sbsmsg->prn,
          type);
...
static void getmet(double lat, double *met) {
  static const double metprm[][10] = {
      ...
      {1015.75, 283.15, 11.66, 5.58E-3, 2.57, -2.25, 11.00, 7.24, 0.32E-3,
       0.46},
      {1011.75, 272.15, 6.78, 5.39E-3, 1.81, -1.75, 15.00, 5.36, 0.81E-3, 0.74},
      {1013.00, 263.65, 4.11, 4.53E-3, 1.55, -0.50, 14.50, 3.39, 0.62E-3,
       0.30}};
  int i, j;
  double a;
  lat = fabs(lat);
...
extern double sbstropcorr(gtime_t time, const double *pos, const double *azel,
                          double *var) {
  const double k1 = 77.604, k2 = 382000.0, rd = 287.054, gm = 9.784,
               g = 9.80665;
  ...
  if (zh == 0.0 || fabs(pos[0] - pos_[0]) > 1E-7 ||
      fabs(pos[1] - pos_[1]) > 1E-7 || fabs(pos[2] - pos_[2]) > 1.0) {
    getmet(pos[0] * R2D, met);
    c = cos(2.0 * PI * (time2doy(time) - (pos[0] >= 0.0 ? 28.0 : 211.0)) /
            365.25);
    ...
    zw *= pow(1.0 - met[3] * h / met[1],
              (met[4] + 1.0) * g / (rd * met[3]) - 1.0);
...
extern int sbssatcorr(gtime_t time, int sat, const nav_t *nav, double *rs,
  ...
  trace(5,
        "sbssatcorr: sat=%2d drs=%6.3f %6.3f %6.3f dclk=%.3f %.3f var=%.3f\n",
