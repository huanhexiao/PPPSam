...
static const char rcsid[] =
    "$Id: convkml.c,v 1.1 2008/07/17 21:48:06 ttaka Exp $";
...
static void outpoint(FILE *fp, gtime_t time, const double *pos,
  ...
  if (outtime) {
    ...
    sprintf(str, "%04.0f-%02.0f-%02.0fT%02.0f:%02.0f:%05.2fZ", ep[0], ep[1],
            ep[2], ep[3], ep[4], ep[5]);
...
static int savekml(const char *file, const solbuf_t *solbuf, int tcolor,
  ...
  char *color[] = {"ffffffff", "ff008800", "ff00aaff",
                   "ff0000ff", "ff00ffff", "ffff00ff"};
  if (!(fp = fopen(file, "w"))) {
  ...
  if (pcolor > 0) {
    ...
    for (i = 0; i < solbuf->n; i++) {
      ...
      outpoint(fp, solbuf->data[i].time, pos, "",
               pcolor == 5 ? qcolor[solbuf->data[i].stat] : pcolor - 1, outalt,
               outtime);
    }
    fprintf(fp, "</Folder>\n");
  }
 ...
 *                               (0:none,1:white,2:green,3:orange,4:red,5:by
 *qflag) int    outalt    I   output altitude (0:off,1:elipsoidal,2:geodetic)
