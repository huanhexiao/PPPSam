 ...
 *     [14] RTCM Standard 10403.2, Differential GNSS (Global Navigation
 *Satellite Systems) Services - version 3, February 1, 2013 [15] RTCM Standard
 *10403.2, Differential GNSS (Global Navigation Satellite Systems) Services -
 *version 3, with amendment 1/2, november 7, 2013 [16] Proposal of new RTCM SSR
 *Messages (ssr_1_gal_qzss_sbas_dbs_v05) 2014/04/17
 *
 ...
 *          to be set to the approximate time within 1/2 hour in order to
 *resolve ambiguity of time in rtcm messages. supported msgs RTCM ver.2:
 *1,3,9,14,16,17,18,19,22 refer [1] for RTCM ver.2
 *-----------------------------------------------------------------------------*/
 ...
 * notes  : before firstly calling the function, time in rtcm control struct has
 *          to be set to the approximate time within 1/2 week in order to
 *resolve ambiguity of time in rtcm messages.
 *
...
extern int input_rtcm3(rtcm_t *rtcm, unsigned char data) {
  ...
  /* check parity */
  if (rtk_crc24q(rtcm->buff, rtcm->len) !=
      getbitu(rtcm->buff, rtcm->len * 8, 24)) {
