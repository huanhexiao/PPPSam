 ...
 *
 * references :
 *     [1] D.D.McCarthy, IERS Technical Note 21, IERS Conventions 1996, July
 *1996 [2] D.D.McCarthy and G.Petit, IERS Technical Note 32, IERS Conventions
 *         2003, November 2003
...
static void tide_pl(const double *eu, const double *rp, double GMp,
  ...
  du += 3.0 / 4.0 * 0.0025 * K2 * sin(2.0 * latp) * sin(2.0 * pos[0]) *
        sin(pos[1] - lonp);
  du += 3.0 / 4.0 * 0.0022 * K2 * cosp * cosp * cosl * cosl *
        sin(2.0 * (pos[1] - lonp));
...
static void tide_oload(gtime_t tut, const double *odisp, double *denu) {
  ...
  a[2] = (270.434358 + 481267.88314137 * t - 0.001133 * t2 + 1.9E-6 * t3) *
         D2R; /* S0 */
  a[3] = (334.329653 + 4069.0340329577 * t - 0.010325 * t2 - 1.2E-5 * t3) *
         D2R; /* P0 */
  a[4] = 2.0 * PI;
...
static void tide_pole(gtime_t tut, const double *pos, const double *erpv,
  ...
  denu[1] =
      -9E-3 * cos(2.0 * pos[0]) * (m1 * cosl + m2 * sinl); /* dn=-Stheta  (m) */
  denu[2] = -33E-3 * sin(2.0 * pos[0]) *
            (m1 * cosl + m2 * sinl); /* du= Sr      (m) */
 ...
 *                                 odisp[0+i*6]: consituent i amplitude
 *radial(m) odisp[1+i*6]: consituent i amplitude west  (m) odisp[2+i*6]:
 *consituent i amplitude south (m) odisp[3+i*6]: consituent i phase radial (deg)
