...
static void SGP4_STR3(double tsince, const tled_t *data, double *rs) {
  ...
  double a1, cosio, theta2, x3thm1, eosq, betao2, betao, del1, ao, delo, xnodp,
      aodp, s4;
  double qoms24, perige, pinvsq, tsi, eta, etasq, eeta, psisq, coef, coef1, c1,
      c2, c3, c4;
  double c5, sinio, a3ovk2, x1mth2, theta4, xmdot, x1m5th, omgdot, xhdot1,
      xnodot;
  double omgcof, xmcof, xnodcf, t2cof, xlcof, aycof, delmo, sinmo, x7thm1, c1sq,
      d2, d3;
  double d4, t3cof, t4cof, t5cof, xmdf, omgadf, xnoddf, omega, xmp, tsq, xnode,
      delomg;
  double delm, tcube, tfour, a, e, xl, beta, xn, axn, xll, aynl, xlt, ayn, capu,
      sinepw;
  double cosepw, epw, ecose, esine, elsq, pl, r, rdot, rfdot, betal, cosu, sinu,
      u, sin2u;
  double cos2u, rk, uk, xnodek, xinck, rdotk, rfdotk, sinuk, cosuk, sinik,
      cosik, sinnok;
...
static int add_data(tle_t *tle, const tled_t *data) {
  ...
  if (tle->n >= tle->nmax) {
    ...
    if (!(tle_data =
              (tled_t *)realloc(tle->data, sizeof(tled_t) * tle->nmax))) {
