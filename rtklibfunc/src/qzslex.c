...
static int decode_lexeph(const unsigned char *buff, int i, gtime_t toe,
  ...
  trace(4, "sat=%2d toe=%s pos=%.3f %.3f %.3f vel=%.5f %.5f %.5f\n", sat,
        time_str(toe, 0), eph.pos[0], eph.pos[1], eph.pos[2], eph.vel[0],
        eph.vel[1], eph.vel[2]);
  trace(4, "clk=%11.3f %8.5f tgd=%7.3f\n", eph.af0 * 1E9, eph.af1 * 1E9,
        eph.tgd * 1E9);
  trace(4, "isc=%6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f\n", eph.isc[0] * 1E9,
        eph.isc[1] * 1E9, eph.isc[2] * 1E9, eph.isc[3] * 1E9, eph.isc[4] * 1E9,
        eph.isc[5] * 1E9, eph.isc[6] * 1E9);
  return i;
}
...
static int decode_lexion(const unsigned char *buff, int i, gtime_t tof,
  ...
  trace(4,
        "t0=%s tspan=%.0f pos0=%.1f %.1f coef=%.3f %.3f %.3f %.3f %.3f %.3f\n",
...
extern int lexreadmsg(const char *file, int sel, lex_t *lex) {
  ...
  if (!(p = strrchr(file, '.')) || (strcmp(p, ".lex") && strcmp(p, ".LEX")))
    return 0;
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    if (sscanf(buff, "%d %d %d", &prn, &type, &alert) == 3 &&
        (p = strstr(buff, ": "))) {
      p += 2;
    } else {
    ...
    if (lex->n >= lex->nmax) {
      ...
      if (!(lex_msgs =
                (lexmsg_t *)realloc(lex->msgs, lex->nmax * sizeof(lexmsg_t)))) {
...
extern int lexeph2pos(gtime_t time, int sat, const nav_t *nav, double *rs,
  ...
  for (i = 0; i < 3; i++) {
    rs[i] =
        eph->pos[i] + eph->vel[i] * t + eph->acc[i] * t2 + eph->jerk[i] * t3;
...
extern int lexioncorr(gtime_t time, const nav_t *nav, const double *pos,
...
#endif
  double tt, sinlat, coslat, sinaz, cosaz, cosel, rp, ap, sinap, cosap, latpp,
      lonpp;
  double dlat, dlon, Enm, F;
  int n, m;

  trace(4, "lexioncorr: time=%s pos=%.3f %.3f azel=%.3f %.3f\n",
        time_str(time, 3), pos[0] * R2D, pos[1] * R2D, azel[0] * R2D,
        azel[1] * R2D);
  ...
  *delay = *var = 0.0;
  ...
  lonpp =
      pos[1] + atan(sinap * sinaz / (cosap * coslat - sinap * cosaz * sinlat));
  ...
  trace(4, "lexioncorr: pos0=%.1f %.1f dlat=%.1f dlon=%.1f\n",
        nav->lexion.pos0[0] * R2D, nav->lexion.pos0[1] * R2D, dlat * R2D,
        dlon * R2D);
    ...
    for (m = 0; m <= 1; m++) {
      ...
      *delay += F * Enm * pow(dlat, n) * pow(dlon, m);
      ...
      trace(5, "lexioncorr: F=%8.3f Enm[%d][%d]=%8.3f delay=%8.3f\n", F, n, m,
            Enm, F * Enm * pow(dlat, n) * pow(dlon, m));
    }
