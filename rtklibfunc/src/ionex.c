...
static int dataindex(int i, int j, int k, const int *ndata) {
  if (i < 0 || ndata[0] <= i || j < 0 || ndata[1] <= j || k < 0 ||
      ndata[2] <= k)
    return -1;
...
static int interptec(const tec_t *tec, int k, const double *posp, double *value,
  ...
  *value = *rms = 0.0;
  ...
  for (n = 0; n < 4; n++) {
    if ((index = dataindex(i + (n % 2), j + (n < 2 ? 0 : 1), k, tec->ndata)) <
        0)
      continue;
    d[n] = tec->data[index];
    r[n] = tec->rms[index];
  ...
  if (d[0] > 0.0 && d[1] > 0.0 && d[2] > 0.0 && d[3] > 0.0) {
    ...
    *value = (1.0 - a) * (1.0 - b) * d[0] + a * (1.0 - b) * d[1] +
             (1.0 - a) * b * d[2] + a * b * d[3];
    *rms = (1.0 - a) * (1.0 - b) * r[0] + a * (1.0 - b) * r[1] +
           (1.0 - a) * b * r[2] + a * b * r[3];
  }
...
static int iondelay(gtime_t time, const tec_t *tec, const double *pos,
  ...
  const double fact = 40.28E16 / FREQ1 / FREQ1; /* tecu->L1 iono (m) */
  double fs, posp[3] = {0}, vtec, rms, hion, rp;
  // double ep[6], var0, var_rem, sigma0, sigma1;
  int i;  // tlc_ippp;
  ...
  *delay = *var = 0.0;
  ...
  for (i = 0; i < tec->ndata[2]; i++) { /* for a layer */
    ...
    // tlc_ippp = loacaltime((int)(ep[3]), posp[1] * R2D);
    ...
    *var += fact * fact * fs * fs * rms * rms;
    /*sigma0 = rms; sigma1 = rms;
    if (tlc_ippp<8 || tlc_ippp>20 || pos[0]>PI/3.0)
            var_rem = SQR(sigma1)*cos(pos[0])*cos((tlc_ippp - 14) / 12 * PI);
    else var_rem = 0.0;
    var0 = (SQR(sigma0) + var_rem) / SQR(sin(azel[1]));
    *var += fact*fact*fs*var0;*/
  }
